...
def plot_scan_results(scan_data, save_path):
    ...
    microns_per_pixel = calculate_scale(x_grid[0], x_grid[-1], 1)
    # Draw the scale bar
    ax.plot([x_grid[0], x_grid[-1]], [y_grid[0], y_grid[0]], color='white', linewidth=2)
    ax.text(x_grid[0] + (x_grid[-1] - x_grid[0])/2, y_grid[0] + 0.02, f'{microns_per_pixel:.1f} µm', color='white', horizontalalignment='center', verticalalignment='bottom')
