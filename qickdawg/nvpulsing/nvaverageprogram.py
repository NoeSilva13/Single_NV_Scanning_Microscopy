...
class NVAveragerProgram(QickRegisterManagerMixin, AcquireProgram):
    ...
    def quick_count(self, integration_time_treg: int, load_pulses=True):
        """
        Performs a single-shot event counting measurement for a specified integration time.
        
        Parameters
        ----------
        integration_time_treg : int
            Integration time in tProc clock cycles
        load_pulses : bool, optional
            Whether to load pulse envelopes (default True)
            
        Returns
        -------
        int
            Total number of counts during the integration period
        """
        # Basic setup and validation
        if not hasattr(self.cfg, 'edge_counting') or not self.cfg.edge_counting:
            raise ValueError("Edge counting must be enabled in configuration")
        
        # Configure readout for edge counting
        self.declare_readout(
            ch=self.cfg.adc_channel,
            length=integration_time_treg,
            freq=0,
            sel='input',
            edge_counting=True,
            high_threshold=self.cfg.high_threshold,
        ...
        # Configure system
        self.config_all(qd.soc, load_pulses=load_pulses, load_mem=True)
        
        # Simple buffer setup for single measurement
        total_count = 1  # Single measurement
        self.d_buf = [np.zeros((1, 2), dtype=np.int64)]  # Simple 1D buffer
        
        # Start single readout
        qd.soc.start_readout(
            total_count,
            counter_addr=self.counter_addr,
            ch_list=[self.cfg.adc_channel],
            reads_per_shot=[1]
        )
        
        # Single poll for data
        while True:
            new_data = obtain(qd.soc.poll_data())
            if new_data:  # Data received
                _, (data, _) = new_data[0]  # Get first (and only) data point
                # Extract the count value from the data array
                count_value = data[0][0][0]  # Access the first element of the 2D array
                return int(count_value)  # Return the count value
